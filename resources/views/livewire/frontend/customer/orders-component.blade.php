<div x-data="{showOrder : @entangle('showOrder')}">
    ...
    <div class="my-4">
        ...
        <div class="border rounded shadow p-4"
             x-show='showOrder' x-on:click.away="showOrder = false">
            ...
            <div class="table-responsive mb-4">
                <table class="table">
                    <thead class="bg-light">
                    <tr>
                        <th class="border-0" scope="col"><strong class="text-small text-uppercase">Transaction</strong>
                        </th>
                    ...
                    @foreach($order->transactions as $transaction)
                        <tr>
                            ...
                            <td>
                                @if ($loop->last
                                       && $transaction->transaction == \App\Models\OrderTransactions::FINISHED
                                       && \Carbon\Carbon::now()->addDays(5)->diffInDays($transaction->created_at->format('Y-m-d')) != 0)

                                    <button wire:click="requestRefundOrder('{!! $order->id !!}')" type="button" class="btn btn-link text-right">
